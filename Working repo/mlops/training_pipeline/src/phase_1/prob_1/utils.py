...
class AppPath:
    ...
    ROOT = Path(os.environ.get("TRAINING_PIPELINE_DIR", "/prob_1"))
    DATA = ROOT / "data"
    DATA_SOURCES = ROOT / "data_sources"
    FEATURE_REPO = ROOT / "feature_repo"
    ARTIFACTS = ROOT / "artifacts"

    PHASE_1 = ARTIFACTS / "phase_1"

    PROBLEM_1 = PHASE_1 / "prob_1"

    TRAINING_PQ_1 = PROBLEM_1 / "training.parquet"
    TRAIN_X_PQ_1 = PROBLEM_1 / "train_x.parquet"
    TRAIN_Y_PQ_1 = PROBLEM_1 / "train_y.parquet"
    TEST_X_PQ_1 = PROBLEM_1 / "test_x.parquet"
    TEST_Y_PQ_1 = PROBLEM_1 / "test_y.parquet"

    RUN_INFO = PROBLEM_1 / "run_info.json"
    EVALUATION_RESULT = PROBLEM_1 / "evaluation.json"

    REGISTERED_MODEL_1_VERSION = PROBLEM_1 / "registered_model_1_version.json"
...
class Config:
    def __init__(self) -> None:
        ...
        self.target_col = os.environ.get("MODEL_1_TARGET_COL")
        self.test_size = float(os.environ.get("MODEL_1_TEST_SIZE"))
        self.experiment_name = os.environ.get("MODEL_1_EXPERIMENT_NAME")
        self.mlflow_tracking_uri = os.environ.get("MLFLOW_TRACKING_URI")
        self.alpha = float(os.environ.get("MODEL_1_ALPHA"))
        self.l1_ratio = float(os.environ.get("MODEL_1_L1_RATIO"))
        self.rmse_threshold = float(os.environ.get("MODEL_1_RMSE_THRESHOLD"))
        self.mae_threshold = float(os.environ.get("MODEL_1_MAE_THRESHOLD"))

        self.registered_model_1_name = os.environ.get("REGISTERED_MODEL_1_NAME")
